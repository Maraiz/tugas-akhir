...
import "../../../styles/dataPikr.css";
import { exportMonevExcel, exportMonevPDF } from "../../../utils/exportMonev";

// Config kolom buat export Excel/PDF — PIK-R paling ramping, cuma 1 angka
// tambahan (jumlahPkbr), gak ada pasangan Anggota/Capaian kayak program lain.
// totalKey di-override manual karena field totalnya "totalPkbr", bukan
// "totalJumlahPkbr" (nggak ngikutin pola otomatis).
const PIKR_COLUMNS = [
    { label: "KODE", key: "kode" },
...
const PIKR_TITLE = "MONEV POKTAN PIK-R YG MELAKSANAKAN EDUKASI KESPRO";
...
function PikrTable() {
    ...
    async function fetchPeriods() {
        ...
        setLoadError("");
        try {
            const response = await api.get("/pikr");
            setPeriods(response.data.data.map(mapPeriodeFromApi));
        } catch (error) {
            console.error(error);
            setLoadError(error.response?.data?.message || "Gagal memuat data PIK-R dari server.");
        } finally {
            setLoading(false);
        }
    }
    ...
    // Total baris "Jumlah Total" di modal Lihat
    const viewTotals = viewDetails.length > 0 ? {
        ...
        totalPkbr: viewDetails.reduce((s, d) => s + (Number(d.jumlahPkbr) || 0), 0),
    } : null;
    ...
    async function confirmDelete() {
        setDeleting(true);
        try {
            await api.delete(`/pikr/${deleteTarget.id}`);
    ...
    return (
        <>
            ...
            <div className="content-toolbar">
                ...
                <Link to="/admin/monitoring/pikr/tambah" className="btn-add-data">
            ...
            {deleteTarget && (
                <div className="modal-overlay open" onClick={(e) => e.target === e.currentTarget && closeDeleteModal()}>
                    <div className="modal-box">
                        ...
                        <p>
                            Seluruh data monitoring PIK-R untuk periode <b>{deleteTarget.periode}</b> ({deleteTarget.jumlahKecamatan} kecamatan)
