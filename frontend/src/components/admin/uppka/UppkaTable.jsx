...
function UppkaTable() {
    ...
    async function fetchPeriods() {
        ...
        setLoadError("");
        try {
            const response = await api.get("/uppka");
            setPeriods(response.data.data.map(mapPeriodeFromApi));
        } catch (error) {
            console.error(error);
            setLoadError(error.response?.data?.message || "Gagal memuat data UPPKA dari server.");
        } finally {
            setLoading(false);
        }
    }
    ...
    async function confirmDelete() {
        setDeleting(true);
        try {
            await api.delete(`/uppka/${deleteTarget.id}`);
    ...
    async function openViewModal(period) {
        ...
        setViewLoading(true);
        try {
            const response = await api.get(`/uppka/${period.id}`);
    ...
    async function saveEdit() {
        ...
        try {
            ...
            await api.put(`/uppka/${editTarget.id}`, { rows: rowsPayload });
    ...
    const editTotals = editRows.length > 0 ? (() => {
        ...
        return {
            totalAda, totalLapor, totalAnggota, totalHadir,
                                    ...
                                    <table className="calc-table">
                                        ...
                                        <tbody>
                                            {editRows.map((d) => {
                                                ...
                                                const pctHadir = anggota > 0 ? (hadir / anggota) * 100 : 0;
                                                return (
                                                    <tr key={d.id}>
                                                        <td>{d.kode ?? "-"}</td>
                                                        <td className="text-left"><b>{d.kecamatan}</b></td>
                                        ...
                                        {editTotals && (
                                            <tfoot>
                                                <tr>
                                                    ...
                                                    <td>{editTotals.totalPctHadir.toFixed(2)}</td>
                                                </tr>
                                            </tfoot>
                                        )}
                                    </table>
