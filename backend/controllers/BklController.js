...
class BklController {
    ...
    // ===========================
    // GET /bkl - list semua periode (buat halaman List)
    ...
    static async index(req, res) {
        ...
        try {
            ...
            const periods = await BklPeriode.findAll({
    ...
    // ===========================
    // GET /bkl/:id - detail 1 periode lengkap dengan baris per kecamatan
    // ===========================
    static async show(req, res) {
        ...
        try {
            ...
            const periode = await BklPeriode.findByPk(id);
            ...
            const details = await BklDetail.findAll({
    ...
    // ===========================
    // POST /bkl - simpan hasil wizard upload (multipart/form-data)
    //
    // Field yang dikirim dari frontend:
    // - file        : file Excel Tabel 5A (diarsipkan, ditangani middleware multer)
    // - bulan       : angka 1-12
    ...
    static async store(req, res) {
        ...
        try {
            ...
            const existing = await BklPeriode.findOne({
            ...
            // dihitung dari pctThdTarget tiap kecamatan
            const rataCapaian = parsedRows.reduce((sum, r) => {
                return sum + (Number(r.pctThdTarget) || 0);
            }, 0) / parsedRows.length;

            const periode = await BklPeriode.create({

                bulan,
                tahun,

                namaFile: req.file ? req.file.originalname : "-",
                pathFile: req.file ? `uploads/bkl/${req.file.filename}` : null,
            ...
            await BklDetail.bulkCreate(detailRecords);

            return res.status(201).json({

                success: true,
                message: "Data monitoring BKL berhasil disimpan.",
    ...
    // ===========================
    // DELETE /bkl/:id - hapus 1 periode beserta seluruh baris kecamatannya
    ...
    static async destroy(req, res) {
        ...
        try {
            ...
            return res.status(200).json({
                ...
                success: true,
                message: "Data periode BKL berhasil dihapus.",
    ...
    // ===========================
    // PUT /bkl/:id - update data per-kecamatan yang sudah tersimpan
    //
    // Body JSON:
    ...
    static async update(req, res) {
        ...
        try {
            ...
            for (const r of rows) {
                ...
                const detail = await BklDetail.findOne({
            ...
            const semuaDetail = await BklDetail.findAll({ where: { periodeId: id } });
